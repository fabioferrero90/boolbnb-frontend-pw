...
const InsertModule = () => {
  ...
  useEffect(() => {
    ...
    if (input && services.length > 0) {
      ...
      tagify.on('change', (e) => {
        ...
        setFormData((prevFormData) => ({ ...prevFormData, services: servicesArray }));
      });
      ...
      if (field.type === "addressAPI") {
        ...
        if (!addressValue) {
          ...
          const promise = getGeocode({ address: addressValue })
            .then((results) => {
              const { newErrors: addressErrors } = validateAddress(results[0].address_components);
              Object.assign(newErrors, addressErrors);
            })
            .catch((error) => {
              if (error.message === "ZERO_RESULTS") newErrors["address"] = "Indirizzo non trovato. Si prega di inserire un indirizzo valido.";
              else newErrors["address"] = "Errore nella geocodifica dell'indirizzo.";
            });
  ...
  const validateAddress = (addressComponents) => {
    const address = {
      ...
      province: getAddressComponent(addressComponents, "administrative_area_level_2", "short_name"),
    ...
    const newErrors = {};
    if (!address.number) newErrors["address"] = "Indirizzo incompleto: Il Numero Civico è obbligatorio";
    else if (!address.address) newErrors["address"] = "Indirizzo incompleto: La Via è obbligatoria";
    else if (!address['postal-code']) newErrors["address"] = "Indirizzo incompleto: Il CAP è obbligatorio";
    else if (!address.city) newErrors["address"] = "Indirizzo incompleto: La Città è obbligatoria";
    else if (!address.province) newErrors["address"] = "Indirizzo incompleto: La Provincia è obbligatoria";
    else if (!address.region) newErrors["address"] = "Indirizzo incompleto: La Regione è obbligatoria";
    else if (!address.country) newErrors["address"] = "Indirizzo incompleto: Lo Stato è obbligatorio";
  ...
  const handleSelect = ({ description }) => () => {
    ...
    getGeocode({ address: description }).then((results) => {
      const { address, newErrors } = validateAddress(results[0].address_components);
  ...
  const getAddressComponent = (components, type) => {
    ...
    return type === "administrative_area_level_2" ? component?.short_name : component?.long_name;
  };
      ...
      <div className="max-w-screen-xl mx-auto md:px-8 py-5">
        <div className="container w-full flex flex-col flex-wrap mx-auto px-2">
          <ol className="flex justify-center items-center w-full mx-auto text-sm font-medium text-center text-gray-100 sm:text-base sticky top-0 bg-white border-2 border-white custom-bg-color-primary py-3 rounded-3xl overflow-hidden">
            {sections.map((section, index) => (
              <React.Fragment key={section.id}>
                {index !== 0 && <div className="md:px-3"><IoMdArrowDroprightCircle /></div>}
                <li className={`flex justify-center items-center w-full ${activeSection === section.id && "custom-teal-color font-bold"}`}>
                  <span className="flex items-center">
                    <a className="inline-flex cursor-pointer" onClick={() => handleMenuClick(section.id)}>
                      {section.menuName}
                    </a>
                  </span>
                </li>
                    ...
                    {section.fields.map((field, idx) => (
                      <div key={idx} className="md:flex mb-6">
                        ...
                        <div className="md:w-3/4">
                          {field.type === "text" && (
                            <input className="px-3 py-1 form-input block w-full rounded-xl border border-gray-300 focus:bg-white" id={field.id} name={field.id} type="text" value={formData[field.id] || ""} onChange={handleFieldChange} ref={(el) => (fieldRefs.current[field.id] = el)} />
                          )}
                          {field.type === "phone" && (
                            <div className="flex">
                          ...
                          {field.type === "addressAPI" && (
                            <div ref={ref}>
                              <input className="px-3 py-1 form-input block w-full rounded-xl border border-gray-300 focus:bg-white" value={value} onChange={handleInput} disabled={!ready} placeholder="" ref={(el) => (fieldRefs.current["address"] = el)} />
                              {status === "OK" && <ul>{renderSuggestions()}</ul>}
                            </div>
                          )}
                          {field.type === "services" && (
                            <div>
                              <input className="customLook px-3 py-1 form-input block w-full rounded-xl border border-gray-300 focus:bg-white custom-tagify-services" name="services" />
                            </div>
                          )}
                          {field.type === "upload_single" && (
                            <div>
                              <input type="file" className="block w-full text-sm text-gray-500 file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-sm file:font-semibold file:bg-purple-50 file:text-purple-700 hover:file:bg-purple-100" onChange={(e) => {
                          ...
                          {field.type === "upload_multiple" && (
                            <div>
                              <input type="file" multiple className="block w-full text-sm text-gray-500 file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-sm file:font-semibold file:bg-purple-50 file:text-purple-700 hover:file:bg-purple-100" onChange={(e) => {
                                ...
                                Promise.all(previews).then((images) => setFormData((prevFormData) => ({ ...prevFormData, [field.id]: images })));
                              }} />
                              {formData[field.id] && formData[field.id].map((src, index) => <img key={index} src={src} alt={`Preview ${index}`} className="mt-2 h-32 w-32 object-cover" />)}
                            </div>
                          )}
                          <p className="py-2 text-sm text-gray-800">{field.description}</p>
          ...
          <div className="flex justify-center items-center bottom-5 gap-5 sticky">
            <button id="actionButton" onClick={handleBackButtonClick} className={`cursor-pointer shadow-4xl bg-gray-300 focus:shadow-outline focus:outline-none text-gray-800 font-bold py-2 px-4 rounded-2xl text-xl ${sections.findIndex((section) => section.id === activeSection) === 0 && "hidden"}`} type="button">
              Indietro
            </button>
            <button id="actionButton" onClick={handleNextButtonClick} className={`cursor-pointer shadow-4xl custom-bg-color-primary focus:shadow-outline focus:outline-none text-white font-bold py-2 px-4 rounded-2xl text-xl`} type="button">
              {sections.findIndex((section) => section.id === activeSection) + 1 < sections.length ? "Avanti" : "Conferma Inserimento"}
            </button>
          </div>
        </div>
      </div>
